...
CREATE SEQUENCE IF NOT EXISTS restaurant.restaurants_id_seq START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS restaurant.restaurants (
    restaurants_id BIGINT PRIMARY KEY DEFAULT nextval('restaurant.restaurants_id_seq'),
    address_id BIGINT NOT NULL REFERENCES restaurant.address(address_id) ON DELETE CASCADE,
    owner_id BIGINT NOT NULL REFERENCES restaurant.users(users_id),
    restaurant_public_id UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
...
ALTER SEQUENCE restaurant.restaurants_id_seq OWNED BY restaurant.restaurants.restaurants_id;

CREATE SEQUENCE IF NOT EXISTS restaurant.menu_items_id_seq START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS restaurant.menu_items (
    menu_items_id BIGINT PRIMARY KEY DEFAULT nextval('restaurant.menu_items_id_seq'),
    restaurant_id BIGINT NOT NULL REFERENCES restaurant.restaurants(restaurants_id) ON DELETE CASCADE,
...
ALTER SEQUENCE restaurant.menu_items_id_seq OWNED BY restaurant.menu_items.menu_items_id;
...
values(5555, '8ff8663d-8379-4089-b756-48fa3a100959','admin', 'adminlogin', 'admin@gmail.com', 'adminlogin123', 'O');
