...
public class UserRepositoryImpl implements UserRepository {
    ...
    @Override
    public boolean findByUserPassword(String user, String password) {
        String sql = """
             SELECT EXISTS(
                 SELECT 1 FROM restaurant.users WHERE login = :user AND password = :password
             )
             """;
            return jdbcTemplate.queryForObject(sql, Map.of("user", user, "password",password), Boolean.class);
    }
}
